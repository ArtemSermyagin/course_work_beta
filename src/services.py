...
def search_transactions(file_path, query):
    ...
    filtered_df = df[df["Описание"].str.contains(query, case=False,
                                                 na=False)]  # Фильтрация DataFrame по запросу в колонке "Описание"

    result = filtered_df.to_dict(orient="records")  # Преобразование фильтрованного DataFrame в список словарей
    log_1.info("Преобразование фильтрованного DataFrame в список словарей")
    return json.dumps(result, indent=4, ensure_ascii=False)  # Преобразование списка в JSON
